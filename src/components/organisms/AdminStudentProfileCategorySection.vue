...
interface Props {
  ...
  title: string
  borderColor: string
  products: AdminStudentProfileProductRow[]
  /** `sectionId` из маршрута списка учеников (`all` | секция материалов). */
  studentsListSectionId: string
  /** `studentId` из маршрута профиля. */
  studentRowId: string
  initiallyExpanded?: boolean
  productAccessStatuses: Record<string, StudentAccessStatusValue>
}

const props = withDefaults(defineProps<Props>(), {
  initiallyExpanded: false,
  productAccessStatuses: () => ({}),
})
...
interface Emits {
  ...
  (
    e: 'deadline-click',
    payload: { productId: string; sectionKey: AdminMaterialSectionId; deadlineLabel: string },
  ): void
}
...
const panelId = computed(() => `admin-student-profile-panel-${props.sectionKey}`)

const foldersCount = computed(() => props.products.length)

const toggleLabel = computed(() =>
  expanded.value ? `Свернуть «${props.title}»` : `Развернуть «${props.title}»`,
)
...
const onDeadlineClick = (productId: string, deadlineLabel: string) => {
  emit('deadline-click', { productId, sectionKey: props.sectionKey, deadlineLabel })
}

const onGeneralAccess = (productId: string) => {
  emit('general-access', { productId, sectionKey: props.sectionKey })
}
</script>

<template>
  <li class="admin-student-profile-category-section">
    <div
      class="admin-student-profile-category-section__trigger"
      :style="{ '--student-profile-section-border': borderColor }"
    >
      <div class="admin-student-profile-category-section__item-left">
        <h2 class="admin-student-profile-category-section__item-title">{{ title }}</h2>
      </div>

      <div class="admin-student-profile-category-section__item-right">
        <div class="admin-student-profile-category-section__stat">
          <img class="admin-student-profile-category-section__icon" :src="folderIcon" alt="" aria-hidden="true" />
          <span class="admin-student-profile-category-section__stat-value">{{ foldersCount }}</span>
        </div>

        <button
          type="button"
          class="admin-student-profile-category-section__accordion-toggle"
          :class="{ 'admin-student-profile-category-section__accordion-toggle_expanded': expanded }"
          :aria-expanded="expanded"
          :aria-controls="panelId"
          :aria-label="toggleLabel"
          @click="toggleExpanded"
        >
          <span class="admin-student-profile-category-section__accordion-toggle__glyph-wrap">
            <svg
              class="admin-student-profile-category-section__accordion-toggle__glyph"
              width="12"
              height="12"
              viewBox="0 0 12 12"
              fill="none"
    ...
    <div
      :id="panelId"
      class="admin-student-profile-category-section__panel-wrap"
      :class="{ 'admin-student-profile-category-section__panel-wrap_expanded': expanded }"
      role="region"
      :aria-label="`Продукты: ${title}`"
    >
      <div class="admin-student-profile-category-section__panel-inner" :inert="!expanded" :aria-hidden="!expanded">
        <div
        ...
        >
          ...
          >
            <button
              ...
              @click="onAddToProduct"
            >
              Добавить на другой продукт
            </button>
          </div>
          ...
          <template v-if="products.length > 0">
            <div
              ...
              :key="product.id"
              class="admin-student-profile-category-section__product-row"
            >
              <RouterLink
                class="admin-student-profile-category-section__product-title admin-student-profile-category-section__product-title_link"
                :style="{ '--student-profile-product-accent': borderColor }"
                :to="{
                ...
                :aria-label="`Управление темами: ${product.title}`"
              >
                {{ product.title }}
              </RouterLink>
              ...
              >
                ...
                <span class="admin-student-profile-category-section__pill-btn-text">Доступ к урокам (модулям)</span>
              </RouterLink>

              <div class="admin-student-profile-category-section__deadline">
                <p class="admin-student-profile-category-section__deadline-label">Дедлайн (общий)</p>
                <button
                  type="button"
                  class="admin-student-profile-category-section__deadline-value"
                  @click="onDeadlineClick(product.id, product.deadlineLabel)"
                >
              ...
              >
                <span class="admin-student-profile-category-section__access-icon" aria-hidden="true">
                  <svg
                    ...
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <circle cx="15" cy="15" r="13" stroke="#00a600" stroke-width="2" />
                  ...
                  <svg
                    ...
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <circle cx="15" cy="15" r="12" fill="#ff8c11" stroke="#ff8c11" stroke-width="2" />
                ...
                <span class="admin-student-profile-category-section__access-text">Общий доступ</span>
              </button>
            </div>
          </template>

          <p v-else class="admin-student-profile-category-section__empty">Нет продуктов в этой секции</p>
        </div>
      </div>
    </div>
  </li>
...
.admin-student-profile-category-section {
  display: flex;
...
.admin-student-profile-category-section__trigger {
  --student-profile-section-border: var(--black);
  border: var(--border-2) solid var(--student-profile-section-border);
...
.admin-student-profile-category-section__item-left {
  display: flex;
  align-items: center;
  gap: var(--sp-20);
  min-width: 0;
...
.admin-student-profile-category-section__stat {
  display: inline-flex;
  align-items: center;
  gap: var(--sp-10);
}
...
.admin-student-profile-category-section__icon {
  width: var(--size-20);
  height: var(--size-20);
  display: block;
  flex-shrink: 0;
...
.admin-student-profile-category-section__accordion-toggle {
  ...
  &:hover:not(.admin-student-profile-category-section__accordion-toggle_expanded) {
    background-color: var(--student-profile-section-border);
    border-color: var(--student-profile-section-border);
    color: var(--white);

    .admin-student-profile-category-section__accordion-toggle__glyph-wrap {
...
.admin-student-profile-category-section__panel-wrap {
  display: grid;
...
.admin-student-profile-category-section__panel-inner {
  min-height: 0;
  overflow: hidden;
}

.admin-student-profile-category-section__panel {
  --student-profile-panel-border: var(--black);
  border: var(--border-2) solid var(--student-profile-panel-border);
  border-radius: var(--radius-10);
...
.admin-student-profile-category-section__panel-head {
  display: flex;
...
.admin-student-profile-category-section__add-btn {
  ...
  background-color: var(--student-profile-panel-border);
  color: var(--white);
  font-family: var(--font-family);
  font-weight: var(--font-medium);
...
.admin-student-profile-category-section__product-title {
  --student-profile-product-accent: var(--main);
  flex: 0 1 221px;
  min-width: 0;
  margin: 0;
  padding: var(--sp-10);
  border: 3px solid var(--student-profile-product-accent);
  border-radius: var(--radius-10);
  background-color: var(--student-profile-product-accent);
  color: var(--white);
  font-family: var(--font-family);
  font-weight: var(--font-medium);
  font-size: 15px;
...
.admin-student-profile-category-section__product-title_link {
...
.admin-student-profile-category-section__pill-btn {
  display: inline-flex;
  align-items: center;
  gap: 15px;
  padding: 5px var(--sp-10);
...
.admin-student-profile-category-section__pill-btn_link {
  text-decoration: none;
  box-sizing: border-box;
}

.admin-student-profile-category-section__pill-btn-icon {
  flex-shrink: 0;
  display: block;
}

.admin-student-profile-category-section__pill-btn-text {
  font-family: var(--font-family);
  font-weight: var(--font-medium);
  font-size: 15px;
  text-align: center;
...
.admin-student-profile-category-section__deadline-label {
  margin: 0;
  width: 100%;
  font-family: var(--font-family);
  font-weight: var(--font-medium);
...
.admin-student-profile-category-section__deadline-value {
  width: 100%;
  margin: 0;
  padding: 5px var(--sp-10);
  border: var(--border-1) solid var(--black);
...
.admin-student-profile-category-section__access-btn {
  display: inline-flex;
  align-items: center;
  gap: 15px;
  padding: 5px var(--sp-10);
...
.admin-student-profile-category-section__access-icon {
  flex-shrink: 0;
  display: flex;
  width: 30px;
  height: 30px;
...
.admin-student-profile-category-section__access-text {
  font-family: var(--font-family);
  font-weight: var(--font-medium);
  font-size: 15px;
  text-align: center;
