...
async function load() {
  ...
  const result = await paymentsService.fail(invId.value)
...
const goToRenewal = () => {
  void router.push({ name: 'home-section', params: { section: 'renewal' } })
}
  ...
  <AppLayout>
    <section class="payment-result">
      <div class="payment-result__card">
        <h1 class="payment-result__title">Оплата не прошла</h1>
        ...
        <template v-else>
          <p class="payment-result__hint">
            {{
              data?.message ||
              'Что-то пошло не так. Деньги не списаны, попробуйте оплатить снова.'
            }}
          </p>
